        ...
        requestQueue.add(getRequest("albums",
            Response.Listener<String> { response ->
                ...
                for (i in 0 until resp.length()) {
                    ...
                    list.add(i, Album(id = item.getString("id"),name = item.getString("name"),
                        cover = item.getString("cover"),
                        recordLabel = item.getString("recordLabel"), releaseDate = fecha,
    ...
    fun getAlbum(albumId: Int, onComplete: (resp: Album) -> Unit, onError: (error: VolleyError) -> Unit){
        requestQueue.add(getRequest("albums/$albumId",
            Response.Listener<String> { response ->
                ...
                var releaseDate = fecha
                val listTrack = mutableListOf<Track>()
                val tracks = resp.getJSONArray("tracks")
                for (j in 0 until tracks.length()) {
                    val itemtrack = tracks.getJSONObject(j)
                        listTrack.add(j, Track(id = itemtrack.getInt("id"),
                        name = itemtrack.getString("name"),duration = itemtrack.getString("duration"),
                        albumId = resp.getInt("id")))

/*
                    val track = Track(id = itemtrack.getInt("id"),
                        name = itemtrack.getString("name"),duration = itemtrack.getString("duration"),
                        albumId = resp.getInt("id"))
*/
                }
                ...
                val albumWithTracks = AlbumWithTracks(album,listTrack)
